...
(() => {
    ...
    const NOTIFICATION_TIMEOUT = 3000;  // 3 seconds for notification display
    const DEBOUNCE_WAIT = 300;  // 300ms for debounced functions
    const DEBOUNCE_SCROLL = 100;  // 100ms for scroll events
    const SCROLL_TOP_THRESHOLD = 100;  // Show scroll-to-top after 100px
    const COUPONS = {
        SAVE10: 0.10,       // 10% off
    ...
    const showNotification = (msg, type = 'info') => {
        ...
        // Auto-hide after notification timeout
        setTimeout(() => {
    ...
    const renderCartItems = () => {
        ...
        cart.forEach(item => {
            ...
            // Create safe HTML structure using DOM elements
            ...
            h3.textContent = item.name;  // Safe from XSS

            const sizeP = document.createElement('p');
            sizeP.textContent = `Size: ${item.size || 'N/A'}`;

            const colorP = document.createElement('p');
            colorP.textContent = `Color: ${item.color || 'N/A'}`;

            const priceP = document.createElement('p');
            priceP.className = 'cart-item-price';
            priceP.textContent = `$${item.price.toFixed(2)}`;

            const stockAlertEl = document.createElement('div');
            if (stockAlert) {
                stockAlertEl.innerHTML = stockAlert;
            }

            const quantityDiv = document.createElement('div');
            quantityDiv.className = 'cart-item-quantity';
            const label = document.createElement('label');
            label.setAttribute('for', `quantity-${item.id}`);
            ...
            wishlistBtn.className = 'btn btn-wishlist wishlist-btn';
            wishlistBtn.dataset.id = item.id;
            wishlistBtn.textContent = wishlistText;

            const sizeGuideBtn = document.createElement('button');
            sizeGuideBtn.className = 'link-btn size-fit-link';
            sizeGuideBtn.setAttribute('aria-disabled', 'true');
            sizeGuideBtn.textContent = 'Size & Fit Guide';

            const sizeFitP = document.createElement('p');
            sizeFitP.appendChild(sizeGuideBtn);

            detailsDiv.appendChild(h3);
            detailsDiv.appendChild(sizeP);
            detailsDiv.appendChild(colorP);
            detailsDiv.appendChild(priceP);
            detailsDiv.appendChild(stockAlertEl);
            detailsDiv.appendChild(quantityDiv);
            detailsDiv.appendChild(removeBtn);
            detailsDiv.appendChild(wishlistBtn);
            detailsDiv.appendChild(sizeFitP);
    ...
    const filterProducts = () => {
        ...
        // Use DocumentFragment for better performance - single reflow instead of many
    ...
    const init = () => {
        ...
        // 7. Navbar dropdown toggle & close on outside click or link click
        ...
        // 8. Scroll-to-top button (debounced)
        if (navbarEls.scrollToTopBtn) {
            window.addEventListener('scroll', debounce(() => {
                navbarEls.scrollToTopBtn.style.display = window.pageYOffset > SCROLL_TOP_THRESHOLD ? 'block' : 'none';
            }, DEBOUNCE_SCROLL));
